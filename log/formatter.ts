...
import { Fields, Log, Level, levelString, allLevels } from "./log.ts";
...
export class TextFormatter {
    ...
    #init = (log: Log): void => {
        if (log.out !== undefined) {
            ...
            if (typeof out.rid === "number") {
                ...
                // Do this to be safe
                const r = Result.of(() => Deno.isatty(out.rid));
                this.#isTerminal = r.success() ?? false;
            }
            else if (typeof out.fd === "number") {
                this.#isTerminal = Deno.isatty(out.fd);
            }
        }
    ...
    #isColored = (): boolean => {
        const isColored = this.forceColors || (this.#isTerminal && Deno.build.os !== "windows");
