...
import * as strings from "../strings/mod.ts";
import { lookup as lookupEnv, set as setEnv } from "./env.ts";
...
function parseLine(line: string, envMap: Map<string, string>): Result<[
    key: string,
    value: string
], error> {
    if (line.length === 0) {
...
function readFile(filename: string): Result<Map<string, string>, error> {
    ...
    try {
        return parse(Deno.readTextFileSync(filename));
    }
    catch (e) {
